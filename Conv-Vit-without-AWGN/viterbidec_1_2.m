...
function [dec_op]=viterbidec_1_2(rcvd)
input=[];
...
%%%A R R A Y S - U P D A T I N G  P A R T
st_hist(1:4, 1:17)=55; %state history array
aem=zeros(4, 17); %accumulated error metric array
...
for (t=0:1:lim) %clock loop
    ...
    if(t==0)
        ...
        %update the accumulative error metrics for all states for
        %current instant 't'
