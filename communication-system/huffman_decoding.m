function decoded_msg = huffman_decoding(unique_symbol, code_word, stream)
% INPUT: 
...
% bit stream pointer 
ptr = 1; 
for i = i_min:length(stream)
    if isempty(find(strcmp(code_word, char(stream(ptr:i) + '0')), 1)) ~= 1
        ind = find(strcmp(code_word, char(stream(ptr:i) + '0')), 1); 
